...
def determine_charge_BVS(get_dict, thre_max):
    ...
    if np.isclose(mean_values, 9999):
        ...
        charge_BVS = 7777
        print("Multiple deltas are less than the threshold value")
    else:
        charge_BVS = sort[0]
        ...
        for idx, spec in enumerate(unique_species):
            if spec[0] == "Metal":
                ...
                if metal.coord in mol.coord:
                    ...
                    possible_charge_metal = list(
                        df[df["atom_1"] == metal.label].atom_1_valence.unique()
                    )

                    if (
                        9 in possible_charge_metal
                    ):  # 9 : OS didn't be specified in the original reference
                    ...
                    print(
                        "Possible charges of Metal {} by BVS : {}".format(
                    ...
                    get_dict.update(
                        (key, round(val, 6)) for key, val in get_dict.items()
                    )
