...
def parsing_input():
    ...
    parser.add_option(
        ...
        "--quiet",
        dest="quiet",
        help="Suppress all but explicitly requested screen output. Overrides --verbose flag.",
        action="store_true",
    )
    # GENERAL OPTIONS
    ...
    generalopts.add_option(
        ...
        "--program",
        dest="program",
        help="The electronic structure code you want to create input file(s) for. Currently supports: "
        + outputprogramstring
    ...
    generalopts.add_option(
        ...
        "--outputfile",
        dest="outputfile",
        help="Name of output file (if other than default for you electronic structure code).",
        metavar="FILE",
    )
    generalopts.add_option(
        "-a",
        "--append",
        dest="append",
        help="Append the output to given output file rather than overwriting.",
        action="store_true",
    )
    generalopts.add_option(
        "--grammar",
        dest="grammar",
        help="Set the CIF grammar to be used when parsing the input file (default is 1.1).",
    )
    generalopts.add_option(
        "--which-filename",
        dest="filenamequery",
        help="If given together with the --program option, the name of the output file will be printed to screen.",
        action="store_true",
    )
    generalopts.add_option(
        "-b",
        "--block",
        dest="block",
        help="Block of data in input file (if there are more than one block in the CIF file).",
    )
    # CELL GENERATION OPTIONS
    cellgenopts = OptionGroup(parser, "Cell generation options")
    cellgenopts.add_option(
        "--no-reduce",
        dest="noreduce",
        help="Do not reduce to the primitive cell.",
        action="store_true",
    )
    cellgenopts.add_option(
        "--force",
        dest="force",
        help="Attempt to force generation of output file despite problems and/or ambiguities in the input file. There are no guarantees that what you get makes sense, but the program makes an honest attempt. Implies --force-alloy.",
        action="store_true",
    )
    cellgenopts.add_option(
        "--force-alloy",
        dest="forcealloy",
        help="Force generation of output file for an alloy compound for an electronic structure code that does not implement any alloy theory (such as CPA).",
        action="store_true",
    )
    cellgenopts.add_option(
        "--vca",
        dest="vca",
        help="Set up an alloy using the virtual crystal approximation (VCA). Currently only supported by the CASTEP interface.",
        action="store_true",
    )
    cellgenopts.add_option(
        "--cartesian",
        dest="cartesian",
        help="Make the program generate any output in cartesian coordinates.",
        action="store_true",
    )
    cellgenopts.add_option(
        "--coordinate-tolerance",
        dest="coordtol",
        help="Parameter for determining when two coordinates are the same (default=0.0002).",
    )
    cellgenopts.add_option(
        "--setup-all",
        dest="setupall",
        help="Make a more complete setup, not just the geometrical part. This is currently only available for "
        + setupallstring
        + ".",
        action="store_true",
    )
    cellgenopts.add_option(
        "--k-resolution",
        dest="kresolution",
        help="The desired resolution in k-space (default=0.2). Used for generating k-space grid options if --setup-all is specified.",
    )
    cellgenopts.add_option(
        "--transform-cell",
        dest="celltransformation",
        help="Transformation matrix applied to the lattice vectors and the symmetry operations if you, for example, want to realign the cell.",
        metavar="[[],[],[]]",
    )
    cellgenopts.add_option(
        "--body-centred-setting",
        dest="specialIsetting",
        help="If set to 1, use the more symmetrical set of primitive translation vectors used for the bcc structure also for other body-centred crystals.",
        metavar="0,1",
    )
    cellgenopts.add_option(
        "--cubic-diagonal-z",
        dest="cubediagz",
        help="Set up cubic cell with [111] direction along the z-axis.",
        action="store_true",
    )
    cellgenopts.add_option(
        "--rhombohedral-diagonal",
        dest="rhombdiag",
        help="Set up rhombohedral cell with threefold axis along pseudocubic [111] direction.",
        action="store_true",
    )
    cellgenopts.add_option(
        "--random-displacements",
        dest="randomdisp",
        help="Randomly displace all atoms. Depending on the distribution, the displacement size is either the maximal displacement (for uniform distribution) or the standard deviation (for gaussian distribution) in Angstrom.",
        metavar="displacementsize",
    )
    cellgenopts.add_option(
        "--random-displacements-distribution",
        dest="randomdistr",
        help="The distribution used for displacing the atoms.",
        metavar="uniform/gaussian",
    )
    cellgenopts.add_option(
        "--export-cif-labels",
        dest="exportlabels",
        help="Export atom labels from the CIF file (currently only supported for castep and RSPt).",
        action="store_true",
    )
    # SUPERCELL OPTIONS
    ...
    supercellopts.add_option(
        "--supercell",
        dest="supercellmap",
        help="Three integers separated with commas and enclosed in square brackets that specify the dimensions of a supercell OR three vectors of integers that gives the map to the supercell to be constructed from the primitive cell. If combined with the --no-reduce option the supercell will instead be generated based on the conventional cell.",
        metavar="[k,l,m]/[[],[],[]]",
    )
    supercellopts.add_option(
        "--supercell-dimensions",
        dest="supercelldims",
        help="Three numbers separated with commas and enclosed in square brackets that specify the desired ABSOLUTE dimensions of a supercell (in angstrom) OR three vectors of numbers that gives the desired lattice vectors. The program will automatically generate a supercell, attempting to get as close as possible to the desired dimensions.",
        metavar="[x,y,z]/[[],[],[]]",
    )
    supercellopts.add_option(
        "--supercell-vacuum",
        dest="supercellvacuum",
        help="Three numbers >=0 separated with commas and enclosed in square brackets that specify a number of unit cell units of vacuum to be added along the first, second or third of the generated lattice vectors.",
        metavar="[k,l,m]",
    )
    supercellopts.add_option(
        "--supercell-translation-vector",
        "--supercell-prevacuum-translation",
        dest="supercellprevactransvec",
        help="Three numbers separated with commas and enclosed in square brackets that specify a shift of all atomic positions in the cell prior to vacuum generation (in units of the lattice vectors of the supercell).",
        metavar="[k,l,m]",
    )
    supercellopts.add_option(
        "--supercell-postvacuum-translation",
        dest="supercellpostvactransvec",
        help="Three numbers separated with commas and enclosed in square brackets that specify a final shift of all atomic positions in the final cell (in units of the lattice vectors of the new cell).",
        metavar="[k,l,m]",
    )
    supercellopts.add_option(
        "--supercell-realign",
        dest="supercellrealign",
        help="Realign the supercell lattice vectors with respect to the cartesian reference frame. For orthorhombic cells, it puts the first, second and third lattice vectors along x, y and z, respectively.",
        metavar="0,1",
    )
    supercellopts.add_option(
        "--supercell-sort",
        dest="supercellsort",
        help="Sort the atom positions by some scheme. Currently available are: \n1) By cartesian coordinate - example: xzy will sort first on x then on z then on y. \n2) by lattice vector - example: 132 will sort first by lattice vector 1 then by lattice vector 3 and last by lattice vector 2.",
    )
    # SURFACE OPTIONS
    surfaceopts = OptionGroup(parser, "Surface generation options")
    surfaceopts.add_option(
        "--surface-wizard",
        dest="surfacewizardhkl",
        help="Three integers separated with commas and enclosed in square brackets that specify a (hkl) plane. The wizard will suggest a supercell map that gives the first two lattice vectors in the (hkl) plane. The third lattice vector is selected as the [hkl] direction, or reasonably orthogonal to the (hkl) plane (if the [hkl] direction is far from orthogonal to this plane).",
        metavar="[h,k,l]",
    )
    # PRINTING OPTIONS
    printopts = OptionGroup(parser, "Printing options")
    printopts.add_option(
        "--print-digits",
        dest="printdigits",
        help="Number of digits used when printing coordinates etc. to screen (default=8). Useful if you need to tweak the screen output for cutting and pasting into some unsupported program. There is no point in going over 16 because of the floating point accuracy.",
    )
    printopts.add_option(
        "--print-atomic-units",
        dest="printau",
        help="Output lattice parameters in bohrradii rather than angstrom.",
        action="store_true",
    )
    printopts.add_option(
        "--print-cartesian",
        dest="printcart",
        help="Atomic sites printed to screen in cartesian rather than lattice coordinates.",
        action="store_true",
    )
    printopts.add_option(
        "--print-symmetry-operations",
        dest="printsymops",
        help="Print symmetry operations of the generated cell.",
        action="store_true",
    )
    printopts.add_option(
        "--print-seitz-matrices",
        dest="printseitz",
        help="Print symmetry operations of the generated cell in Seitz matrix form.",
        action="store_true",
    )
    printopts.add_option(
        "--print-charge-state",
        ...
        dest="printcharges",
        help="Print information about the oxidation state from the CIF file.",
        action="store_true",
    )
    printopts.add_option(
        "--print-reference-bibtex",
        dest="bibtexref",
        help="Print citation in BibTeX format and exit.",
        action="store_true",
    )
    ...
    progspec.add_option(
        "--abinit-braces",
        dest="abinitbraces",
        help="Put curly braces around input values for ABINIT.",
        action="store_true",
    )
    # cellgen
    progspec.add_option(
        "--cellgen-map",
        dest="cellgenmap",
        help="Nine integers separated with commas and enclosed three and three in square brackets (this is a matrix in Python) that specify the map to a supercell to be output for the RSPt supercell generator 'cellgen'. Overrides --cellgen-supercell-dims.",
        metavar="[[k,l,m],[n,o,p],[q,r,s]]",
    )
    progspec.add_option(
        "--cellgen-supercell-dimensions",
        dest="cellgensupercelldims",
        help="Three integers separated with commas and enclosed in square brackets that specify the dimensions of a supercell to be output to the RSPt supercell generator 'cellgen' (the diagonal elements of the 'map').",
        metavar="[k,l,m]",
    )
    progspec.add_option(
        "--cellgen-reference-vector",
        dest="cellgenrefvec",
        help="Three reals separated with commas and enclosed in square brackets that specify an optional shift of the origin used by the RSPt supercell generator 'cellgen'.",
        metavar="[x,y,z]",
    )
    # CASTEP
    progspec.add_option(
        "--castep-cartesian",
        dest="castepcartesian",
        help="Output atom positions in cartesian rather than lattice coordinates.",
        action="store_true",
    )
    progspec.add_option(
        "--castep-atomic-units",
        dest="castepatomicunits",
        help="Output to CASTEP in atomic units (bohr radii) rather than angstrom.",
        action="store_true",
    )
    # CPMD
    progspec.add_option(
        "--cpmd-cutoff",
        dest="cpmdcutoff",
        help="Set the cutoff written to the &SYSTEM block (default=100.0 Ry).",
    )
    # Crystal09
    progspec.add_option(
        "--crystal09-rhombohedral-setting",
        dest="crystal09rhombohedral",
        help="For trigonal spacegroups where this is possible, specify the rhombohedral cell in the Crystal09 input.",
        action="store_true",
    )
    # emto
    progspec.add_option(
        "--emto-hard-sphere-radii",
        dest="hardsphereradii",
        help="Set hard spheres in KSTR to something other than the default (=0.67).",
    )
    # FHI-AIMS
    progspec.add_option(
        "--fhi-aims-cartesian",
        dest="aimscartesian",
        help="Store the coordinates for FHI-AIMS in cartesian format.",
        action="store_true",
    )
    # MOPAC
    progspec.add_option(
        "--mopac-first-line",
        dest="mopacfirstline",
        help="String to be used for the first line (the run commands) of the MOPAC input.",
        metavar='"string"',
    )
    progspec.add_option(
        "--mopac-second-line",
        dest="mopacsecondline",
        help="String to be used for the second line (documentation) of the MOPAC input.",
        metavar='"string"',
    )
    progspec.add_option(
        "--mopac-third-line",
        dest="mopacthirdline",
        help="String to be used for the third line (documentation) of the MOPAC input.",
        metavar='"string"',
    )
    progspec.add_option(
        "--mopac-freeze-structure",
        dest="mopacfreeze",
        help="If set to 'T' then add a 0 after each coordinate (freezing the structure), if set to 'F' then add a 1 (allowing everything to relax).",
        metavar="T/F",
    )
    # PWSCF
    progspec.add_option(
        "--pwscf-pseudostring",
        dest="pwscfpseudostring",
        help='String to attach to the element name to identify the pseudopotential file (e.g. something like "_HSCV_PBE-1.0.UPF").',
        metavar="_PSEUDO",
    )
    progspec.add_option(
        "--pwscf-atomic-units",
        dest="pwscfatomicunits",
        help="Write PWSCF .in file in atomic units (bohr) rather than angstrom.",
        action="store_true",
    )
    progspec.add_option(
        "--pwscf-alat-units",
        dest="pwscfalatunits",
        help="Use 'alat' units for the positions in the PWSCF .in file.",
        action="store_true",
    )
    progspec.add_option(
        "--pwscf-cartesian",
        dest="pwscfcart",
        help="Write lattice vectors and positions to PWSCF .in file in cartesian coordinates and set the lengths scale to 1.",
        action="store_true",
    )
    progspec.add_option(
        "--pwscf-cartesian-latticevectors",
        dest="pwscfcartvects",
        help="Write lattice vectors to PWSCF .in file in cartesian coordinates and set the lengths scale to 1.",
        action="store_true",
    )
    progspec.add_option(
        "--pwscf-cartesian-positions",
        dest="pwscfcartpos",
        help="Write lattice positions to PWSCF .in file in cartesian coordinates.",
        action="store_true",
    )
    # RSPt
    progspec.add_option(
        "--rspt-new",
        dest="newsymt",
        help="Generate a symt.inp file in the new format.",
        action="store_true",
    )
    progspec.add_option(
        "--rspt-spinpol",
        dest="rsptspinpol",
        help="Generate new format symt.inp file with spin polarization.",
        action="store_true",
    )
    progspec.add_option(
        "--rspt-relativistic",
        dest="rsptrelativistic",
        help="Generate new format symt.inp file with relativistic effects.",
        action="store_true",
    )
    progspec.add_option(
        "--rspt-spinaxis",
        dest="rsptspinaxis",
        help="Spin axis for symt.inp (default is [0.0,0.0,0.0].",
        metavar="[x,y,z]",
    )
    progspec.add_option(
        "--rspt-no-spin",
        dest="rsptnospin",
        help="Force a nonmagnetic setup in conjunction with --setup-all.",
        action="store_true",
    )
    progspec.add_option(
        "--rspt-mtradii",
        dest="rsptmtradii",
        help="Integer that gives the method for setting muffin tin radii.",
        metavar="N",
    )
    progspec.add_option(
        "--rspt-cartesian-latticevectors",
        dest="rsptcartlatvects",
        help="Put lattice vectors in atomic units and the lenght scale parameter to 1.",
        action="store_true",
    )
    progspec.add_option(
        "--rspt-pass-wyckoff",
        dest="rsptpasswyckoff",
        help="Pass wyckoff labels from CIF file to the symt/rspt.inp file.",
        action="store_true",
    )
    # SPRKKR/xband
    progspec.add_option(
        "--sprkkr-minangmom",
        dest="sprkkrminangmom",
        help="Enforce minimum onsite angular momentum (=l+1, so that 3 will be d-states).",
    )
    # spacegroup
    progspec.add_option(
        "--spacegroup-supercell",
        dest="spacegroupsupercell",
        help="Three integers separated with commas and enclosed in square brackets that specify the dimensions of a supercell to be output to the elk input generator 'spacegroup'.",
        metavar="[k,l,m]",
    )
    # VASP
    progspec.add_option(
        "--vasp-format",
        dest="vaspformat",
        help="Format of the generated POSCAR file, either 4 or 5. Default is 4.",
    )
    progspec.add_option(
        "--vasp-print-species",
        dest="vaspprintspcs",
        help="Print the atomic species to screen in the order they are put in the POSCAR file (useful for scripting).",
        action="store_true",
    )
    progspec.add_option(
        "--vasp-cartesian",
        dest="vaspcart",
        help="Write lattice vectors and positions to POSCAR file in cartesian coordinates and set length to 1.",
        action="store_true",
    )
    progspec.add_option(
        "--vasp-cartesian-lattice-vectors",
        dest="vaspcartvecs",
        help="Write lattice vectors to POSCAR file in cartesian coordinates and set the length scale to 1.",
        action="store_true",
    )
    progspec.add_option(
        "--vasp-cartesian-positions",
        dest="vaspcartpos",
        help="Write atomic positions to POSCAR file in Cartesian rather than Direct coordinates.",
        action="store_true",
    )
    progspec.add_option(
        "--vasp-selective-dynamics",
        dest="vaspselectivedyn",
        help="Output POSCAR in selective dynamics format (without any constrained atoms).",
        action="store_true",
    )
    progspec.add_option(
        "--vasp-pseudo-libdr",
        dest="vasppseudolib",
        help="Path to the VASP pseudopotential library. Also settable by the VASP_PAWLIB environment variable.",
    )
    progspec.add_option(
        "--vasp-pseudo-priority",
        dest="vasppppriority",
        help="Set the priority of different pseudopotentials by a list of suffixes. Also available via the VASP_PP_PRIORITY environment variable.",
        metavar='"_d,_pv,_sv,_h,_s"',
    )
    progspec.add_option(
        "--vasp-encutfac",
        dest="vaspencutfac",
        help="Factor that multiplies the maximal ENCUT found in the POTCAR file.",
        metavar="1.5",
    )
    # xyz
    progspec.add_option(
        "--xyz-atomic-units",
        dest="xyzatomicunits",
        help="Output xyz file in atomic units (bohr radii) rather than angstrom.",
        action="store_true",
    )
    #
...
def cif_2_info(input_path,infopath,errorpath,angtobohr=angtobohr,codename=codename,uperautogpercm=uperautogpercm):
    ...
    # Make supercell?
    if (
        options.supercellmap
        or options.supercelldims
        or options.supercellvacuum
        or options.supercellprevactransvec
        or options.supercellpostvactransvec
    ):
        makesupercell = True
    else:
        makesupercell = False
    ...
    if verbose or not options.program and not options.quiet:
        ...
        if options.printcart:
            ...
            for i in range(3):
                ...
                for j in range(3):
                    transmtx[i].append(
                        inputcell.latticevectors[i][j] * inputcell.lengthscale
                    )
                i += 1
            for i in ["x", "y", "z"]:
        ...
        try:
            ...
            infofilename.write(
                ...
                + volstring
                + "^3 = "
                + decform % (density * uperautogpercm)
                + " g/cm^3\n"
            )
        except:
    ...
    if transformcell:
        ...
        if options.cubediagz:
            ...
            celltransformation = LatticeMatrix(
                [
    ...
    if makesupercell:
        ...
        try:
            cd.getSuperCell(
                supercellmap,
                supercellvacuum,
                supercellprevactransvec,
                postvactransvec=supercellpostvactransvec,
                sort=supercellsort,
    ...
    if not (makesupercell and supercellsort):
        try:
            cd.atomdata.sort(
                key=lambda a: ed.elementnr[max(a[0].species, key=a[0].species.get)],
                reverse=True,
            )
        except:
            pass
    ...
    def StandardDocstring():
        ...
        stringlen = max(
            len(ref.referencestring()),
            len(ref.cpd + "   (" + ref.compound + ")"),
            len(cif2cellstring),
        )
        docstring = ""
        tmpstring = ""
        ...
        if ref.database != "":
            ...
            if ref.databasecode != "" and ref.databasecode is not None:
    ...
    if outputprogram == "cif":
        ...
        if (
            makesupercell
            or (reducetoprim and cd.spacegroupsetting != "P")
            or options.randomdisp
        ):
            ...
            cb["_cell_angle_alpha"] = (
                acos(b.dot(c) / (b.length() * c.length())) * 180 / pi
            )
            cb["_cell_angle_beta"] = (
                acos(a.dot(c) / (a.length() * c.length())) * 180 / pi
            )
            cb["_cell_angle_gamma"] = (
                acos(b.dot(a) / (a.length() * b.length())) * 180 / pi
            )
    ...
    if outputprogram == "crystal09":
        ...
        crystal09file = Crystal09File(
            cd, docstring, rhombohedral=options.crystal09rhombohedral
        )
    ...
    if (
        outputprogram == "emto"
        or outputprogram == "kgrn"
        or outputprogram == "kfcd"
        or outputprogram == "kstr"
        or outputprogram == "bmdl"
        or outputprogram == "shape"
    ):
        # Get job names
        if cd.HMSymbol == "":
        ...
        if (
            len(kstrjobnam) > 30
            and len(cif_file_name) < len(kstrjobnam) + 4
            or kstrjobnam == ""
        ):
            kstrjobnam = cif_file_name.replace(".cif", "")
        if (
            len(kgrnjobnam) > 30
            and len(cif_file_name) < len(kgrnjobnam) + 4
            or kgrnjobnam == ""
        ):
    ...
    if outputprogram == "mopac":
        ...
        mopacinput = MOPACFile(
            ...
            docstring,
            setupall=mopacsetup,
            firstline=mopacfirstline,
            secondline=mopacsecondline,
            thirdline=mopacthirdline,
            freeze=mopacfreeze,
        )
        f = open(outputfile, outmode)
    ...
    if outputprogram == "vasp":
        ...
        if (
            ref.database != ""
            and ref.databasecode != ""
            and ref.databasecode is not None
        ):
            docstring += (
                " from "
                + ref.databaseabbr[ref.database]
                + " reference: "
                + ref.databasecode
            )
        docstring += ". "
        ...
        if setupall:
            ...
            if options.vaspencutfac:
                ...
                encutfac = 1.5
            potcarfile = POTCARFile(
                cd, directory=lib, vca=options.vca, prioritylist=prioritylist
            )
            f = open("POTCAR", "w")
