...
def set_charge_state(unique_specie, target, debug: int = 0):
    ...
    dummy1, dummy2, map12 = reorder(
        ref_data, target_data, unique_specie.coord, target.coord
    )

    if np.array_equal(map12, np.arange(len(target_data))):
        if debug >= 1:
            print(
                f"({target.subtype}) {target.formula} {final_charge=} No need to reorder"
            )
        ...
        if debug >= 1:
            print(
                f"({target.subtype}) {target.formula} {final_charge=} Reordered {map12=}"
            )
...
def get_charge(charge: int, prot: object, allow: bool = True, debug: int = 0):
    ...
    if len(mols) > 1:
        print("WARNING: More than 1 mol received from xyz2mol for initcharge:", charge)
    ...
    ch_state = charge_state.from_positional(
        iscorrect, total_charge, atom_charge, mols[0], smiles, charge, allow, prot
    )
