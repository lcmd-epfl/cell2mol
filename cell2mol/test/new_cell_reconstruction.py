...
def apply_symmetry_operations_reference(
    reference, cell_vector, sym_ops, normalize=True
):
...
######################################################
def reconstuct(
    reference, newcell, cell_pos, cell_fracs, cell_vector, sym_ops, debug: int = 0
):
    new_structures = apply_symmetry_operations_reference(
        reference, cell_vector, sym_ops
    )
        ...
        for i, ref in enumerate(newcell.refmoleclist):
            ...
            for j, rem_frag in enumerate(remaining_fragments):
                ...
                if small_set.issubset(target_set):
                    if debug >= 1:
                        print(
                            f"{j} {rem_frag.formula} is a subset of target_set {i} {ref.formula}"
                        )
