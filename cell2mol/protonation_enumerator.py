...
def enumerate_protonation_states(specie: Specie, debug: int = 0) -> list[Protonation]:
    ...
    for g in ligand.groups:
        ...
        if g.is_haptic:  # HAPTIC GROUPS
            result = _handle_haptic_group(ligand, g, parent_indices, debug)
        else:  # NON-HAPTIC GROUPS
            result = _handle_non_haptic_group(ligand, g, parent_indices, debug)
...
def get_empty_protonation_state(specie: Specie) -> list[Protonation]:
    ...
    charge-state enumeration (using xyz2mol), where a Protonation
    object is required even when no atoms are added.
    """
    logger.debug(
...
def _handle_haptic_group(ligand, g, parent_indices, debug=0) -> ProtonationGroupResult:
    """
    ...
    if debug >= 2:
        print("        HANDLE_HAPTIC_GROUP:", g.haptic_type)
    ...
    # --------------------------------------------------
    # Fully aromatic / saturated rings (block only)
    # --------------------------------------------------
    elif (
...
def _handle_non_haptic_group(
    ...
    for idx in parent_indices:
        ...
        if debug >= 2:
            print(
                f"        HANDLE_NON_HAPTIC: idx={idx}, label={a.label}, "
                f"connec={a.connec}, mconnec={a.mconnec}"
            )
