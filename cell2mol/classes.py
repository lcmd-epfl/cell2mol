...
class specie(object):
    ...
    ############
    def set_atoms(self, atomlist: list=None, create_adjacencies: bool=False, atom_site_labels: list=None, geom_bond_cif: list=None, debug: int = 0):
        debug = 0
        ...
        if atom_site_labels is not None:
            if debug >= 2: print(f"SPECIE.SET_ATOMS: received {atom_site_labels=}")
            self.atom_site_labels = atom_site_labels.copy()
            for idx, at in enumerate(self.atoms):
                at.set_atom_site_label(atom_site_labels[idx])
            
        if create_adjacencies:
            if not hasattr(self,"adjmat"):  self.get_adjmatrix(geom_bond_cif)
            if not hasattr(self,"madjmat"): self.get_metal_adjmatrix(geom_bond_cif)
...
class molecule(specie):
    ...
    def split_IA_IIA(self, debug: int=0):
        ...
        if not self.has_IA_IIA:        self.ligands = None; self.metals = None
            ...
            rest_atom_site_labels = None
            if hasattr(self, "atom_site_labels") and self.atom_site_labels is not None:  
            ...
            if len(rest_labels) == 0:
                ...
                print(f"SPLIT COMPLEX: received {len(blocklist)} blocks")
                if debug > 1:    
                    print(f"SPLIT COMPLEX: received {blocklist=}")
    ...
    def split_complex(self, debug: int=0):
        ...
        if not self.iscomplex:        self.ligands = None; self.metals = None
            ...
            if len(rest_labels) == 0:
                if debug > 0: print(f"SPLIT COMPLEX: No ligands found in the complex {self.formula}")
                if debug > 0: print(f"SPLIT COMPLEX: Returning empty ligand list {self.ligands=}")
                for m in metal_idx:                     
                    self.metals.append(self.atoms[m])  
            else :
                if refcell is not None and getattr(refcell, "exist_cif_bond_moiety", False) and geom_bond_cif is not None:
    ...
    def balance_charges_for_molecules(self, input_charge: int=None, debug: int=0):
        if not hasattr(self, "unique_species"): self.get_unique_species()
        if not hasattr(self, "selected_cs"): self.get_selected_cs()
...
class atom(object):
    ...
    def check_connectivity(self, other: object, debug: int=0):
        ...
        if refcell is not None and getattr(refcell, "exist_cif_bond_moiety", False):
            ...
            isgood, adjmat, adjnum = get_adjmatrix_from_cif_bonds(labels, coords, atom_site_labels, geom_bond_cif)
        else:    
...
class metal(atom,specie):
    ...
    def get_coordination_geometry(self: object, debug: int = 0):
        ...
        if not hasattr(self,"metals"): self.get_connected_metals(debug=debug)
...
class cell(object):
    ...
    #######################################################
    def get_reference_molecules_from_moiety(self, ref_labels: list, ref_fracs: list, cov_factor: float = 1.3, metal_factor: float = 1.0, debug: int = 0):
