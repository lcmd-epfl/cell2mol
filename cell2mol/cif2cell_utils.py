...
class GeometryObject:
    ...
    def __init__(self, compeps=0.0002):
        ...
        self.invcompeps = 1.0 / self.compeps
...
class Vector(list, GeometryObject):
    ...
    * Supports addition with another Vector, in which case the
    ...
    def __hash__(self):
        t = (
            self.invcompeps * self[0]
            + 1.1 * self.invcompeps * self[1]
            + 1.2 * self.invcompeps * self[2]
        )
        return int(round(t))
...
class LatticeVector(Vector):
    ...
    def __add__(self, other):
        if (
            ...
            or self.interval[1] != other.interval[1]
        ):
            raise GeometryObjectError(
                "LatticeVectors must have the same definition intervals to be added."
            )
...
class AtomSite(GeometryObject):
    ...
    def __hash__(self):
        return hash(
            hash(self.position)
            + hash("".join(sorted(self.species.keys())))
            + hash(sum(self.species.values()))
        )
...
class SymmetryOperation(GeometryObject):
    ...
    def __eq__(self, other):
        ...
        for i in range(3):
            for j in range(3):
                eq = (
                    eq
                    and abs(self.rotation[i][j] - other.rotation[i][j]) < self.compeps
                )
    ...
    def diagonal(self):
        if (
            abs(self.rotation[0][1]) < self.compeps
            and abs(self.rotation[0][2]) < self.compeps
            and abs(self.rotation[1][0]) < self.compeps
            and abs(self.rotation[1][2]) < self.compeps
            and abs(self.rotation[2][0]) < self.compeps
            and abs(self.rotation[2][1]) < self.compeps
        ):
            return True
        else:
            return False
...
def minv3(m):
    ...
    w = [
        [
            ...
            (m[0][2] * m[2][1] - m[0][1] * m[2][2]) * di,
            (m[0][1] * m[1][2] - m[0][2] * m[1][1]) * di,
        ],
        [
...
def SurfaceWizard(cell, hkl):
    ...
    suggestedvs = [
        lvs[0].scalmult(inplaneNs[0][1][0])
        + lvs[1].scalmult(inplaneNs[0][1][1])
        + lvs[2].scalmult(inplaneNs[0][1][2])
    ]
    ...
    for n in inplaneNs[1:]:
        ...
        if ang < pi / 4:
            ...
            suggestedvs.append(
                lvs[0].scalmult(n[1][0])
                + lvs[1].scalmult(n[1][1])
                + lvs[2].scalmult(n[1][2])
            )
            break
