...
import numpy as np
from cell2mol.other import get_angle
...
def get_missingH_from_adjacency(Z, center, points, bonded_atom_labels):
    ...
    if val_e == 1 and missingH:
        report += str(
            f" -with a single adjacent atom {bonded_atom_labels[0]} that is not O or N, possibly a methyl group with missing H atoms.\n"
        )
    else:
        report += str(
            f" -arranged in a shape {shape} that suggests coordination {shapeval} \n"
        )
    report += report_shape
...
def check_missingH(refmoleclist: list, debug: int = 0):
    ...
    for idx, ref in enumerate(refmoleclist):
        if (
            ...
            if ref.natoms == 1 and "O" in ref.labels:
                ...
                if debug >= 1:
                    print(
                        "WARNING found isolated O atom in the cell. This tends to be a water with missing H, so stopping"
                    )
                ...
                if debug >= 1:
                    print(
                        f"Found fullerene {ref.formula} in the cell. skipping missing H check in carbon atoms"
                    )
            else:
                for kdx, a in enumerate(ref.atoms):
                    if a.adjacency is None:
                        continue
                    if a.label == "C":
                        bonded_atom_coord = []
                        bonded_atom_labels = []
                        ...
                        if ismissingH:
                            ...
                            if debug >= 1:
                                print(
                                    f"WARNING in Missing H function for: {ref.type}, molecule index {idx}, {ref.formula}"
                                )
                            if debug >= 1:
                                print(
                                    f"C Atom {kdx} {a.get_parent_index('molecule')} ref_idx={a.get_parent_index('reference')} has missing H atoms"
                                )
                            if debug >= 1:
                                print(report)
                            Missing_H_in_C = True
            ...
            for jdx, lig in enumerate(ref.ligands):
                if lig.natoms == 1 and "O" in lig.labels and lig.denticity <= 1:
                    ...
                    for kdx, a in enumerate(lig.atoms):
                        if (
                            a.label == "C" and a.mconnec == 0
                        ):  # Check carbon not connected to metals
                            bonded_atom_coord = []
                            bonded_atom_labels = []
                            for adj in a.adjacency:
                                bonded_atom_coord.append(
                                    lig.get_parent("molecule").coord[adj]
                                )
                                bonded_atom_labels.append(
                                    lig.get_parent("molecule").atoms[adj].label
                                )
                            if debug >= 2:
                                print("Adjacency", a.adjacency, bonded_atom_labels)
                            ismissingH, report, num_missingH = (
                                get_missingH_from_adjacency(
                                    a.atnum,
                                    a.coord,
                                    bonded_atom_coord,
                                    bonded_atom_labels,
                            ...
                            if ismissingH:
                                ...
                                if debug >= 1:
                                    print(
                                        f"WARNING in Missing H function for: {ref.type}, molecule index {idx}, ligand index {jdx}, {lig.formula}"
                                    )
                                if debug >= 1:
                                    print(
                                        f"C Atom {kdx} {a.get_parent_index('molecule')} ref_idx={a.get_parent_index('reference')} has missing H atoms"
    ...
    if Missing_H_in_C or Missing_H_in_CoordWater or Missing_H_in_Water:
        Warning = True
    if not Warning:
        if debug >= 1:
            print("Not a Single Molecule has Missing H atoms (apparently)")
