...
def get_molecule (input_path, name, input_charge, current_dir, debug=2):
    ...
    with open(output_fname, "w") as output:
        with redirect_stdout(output):
            ...
            if newmolec.iscomplex: 
                if debug >= 0: print(f"GET_MOLECULE: working with {newmolec.formula} with transition metals")
                ...
                if debug >= 0: print(f"GET_MOLECULE: working with {newmolec.formula} with alkali or alkali earth metals")
                if len(newmolec.ligands) == 0 :
                    pass
                else:
                    for lig in newmolec.ligands:
