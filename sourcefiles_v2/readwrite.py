...
def readinfo(filepath):
    ...
    strings = ["Lattice parameters:", "Representative sites :", "OUTPUT CELL INFORMATION", "Bravais lattice vectors :", "All sites, (cartesian coordinates):", "Unit cell volume" ]
    lint = np.zeros((len(strings)))

    for l, line in enumerate(lines):
        for stx, string in enumerate(strings):
            if string in line: lint[stx] = l
...
def readcif(filepath):
    ...
    strings = ["_journal_name_full", "_chemical_name_systematic", "_cell_volume", "_atom_type_radius_bond", "_atom_site_label", "_chemical_name_common"]
    lint = np.zeros((len(strings)))
...
def savemolecules(moleclist, output_dir, print_types, option_print_repeated = True):
    ...
    if "xyz" not in print_types: print_xyz = False
    if "gmol" not in print_types: print_gmol = False
    if "npy" in print_types: print_npy = True
    if "mol" in print_types: print_mol = True
    if "txt" in print_types: print_txt = True
    if "dict" in print_types: print_dict = True
    ...
    for mol in moleclist:
        ...
        if shalliprint:
            ...
            if print_xyz: print_molecule(mol, mol.name, "xyz", output_dir)
            if print_gmol: print_molecule(mol, mol.name, "gmol", output_dir)
            if print_npy: print_molecule(mol, mol.name, "npy", output_dir)
            if print_txt: print_molecule(mol, mol.name, "txt", output_dir)
            if print_dict: print_molecule(mol, mol.name, "dict", output_dir)
            if hasattr(mol, 'object'):
                if print_mol: print_molecule(mol, name, "mol", output_dir)  
