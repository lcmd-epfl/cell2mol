...
for i in elemdatabase.elementsym:
    try :
        valence_electrons.append(elemdatabase.valenceelectrons[elemdatabase.elementsym[i]])
    except KeyError:
        continue

atomic_valence_electrons = dict(
    zip(elemdatabase.elementsym, valence_electrons)
)
...
def clean_charges(mol):
    ...
    for i, fragment in enumerate(fragments):
        for smarts in rxn_smarts:
            patt = Chem.MolFromSmarts(smarts.split(">>")[0]) #Construct a molecule from a SMARTS string.
...
def xyz2AC_huckel(atomicNumList, xyz, charge):
    ...
    opop = result.GetReducedOverlapPopulationMatrix() #The reduced overlap population matrix provides the Mulliken overlap population between atoms in the molecule. It's returned as a vector representing a symmetric matrix
